...
Adopts [`circle-ir@4.9.4`](https://www.npmjs.com/package/circle-ir) — C#/.NET (experimental): flags AES/3DES ECB mode (`CipherMode.ECB`) as weak crypto (CWE-327). No CLI surface change.

## [4.9.3] - 2026-08-22

Adopts [`circle-ir@4.9.3`](https://www.npmjs.com/package/circle-ir) — C#/.NET (experimental): flags JWT signature verification disabled (`RequireSignedTokens = false` / bypass `SignatureValidator`, CWE-347) and insecure cookies (`CookieOptions { Secure = false / HttpOnly = false }`, CWE-614). No CLI surface change.

## [4.9.2] - 2026-08-21

Adopts [`circle-ir@4.9.2`](https://www.npmjs.com/package/circle-ir) — C#/.NET (experimental): flags TLS certificate verification disabled via an always-true `ServerCertificateValidationCallback` / `DangerousAcceptAnyServerCertificateValidator` (CWE-295). No CLI surface change.
...
Adopts [`circle-ir@4.4.0`](https://www.npmjs.com/package/circle-ir) — C#/.NET (experimental) now detects the ADO.NET `cmd.CommandText = "…" + x` SQL sink (cognium-ai#289). No CLI surface change.

## [4.3.1] - 2026-08-10

Adopts [`circle-ir@4.3.1`](https://www.npmjs.com/package/circle-ir) — fixes the root-exported `SupportedLanguage` type, which was missing `'csharp'` (a type-only bug; the engine already analyzed C#). No CLI surface change.

## [4.3.0] - 2026-08-10

Adopts [`circle-ir@4.3.0`](https://www.npmjs.com/package/circle-ir) — the redundant clickjacking-header finding pair (`missing-x-frame-options` + `missing-csp-frame-ancestors`) now emits as one finding (cognium-dev#262). Experimental C#/.NET support documented. No CLI surface change.

## [4.2.0] - 2026-08-10

Adopts [`circle-ir@4.2.0`](https://www.npmjs.com/package/circle-ir) — SSRF via Next.js/Remix destructured route params (`params.<seg>`) is now detected (cognium-ai#277). No CLI surface change.
...
Adopts [`circle-ir@3.218.0`](https://www.npmjs.com/package/circle-ir) — `fetch`/`axios` in a browser client component is no longer flagged as SSRF (cognium-ai#279 R-3), gated to leave server-side fetches firing. No CLI surface change.
...
Adopts [`circle-ir@3.215.0`](https://www.npmjs.com/package/circle-ir) — two precision fixes from the skillsregistry audit: XSS no longer fires on Python `print()` (skillsregistry#51), and SSRF no longer fires on SDK entity lookups like `client.channels.fetch(id)` (skillsregistry#49). No CLI surface change.

## [3.214.0] - 2026-08-07

Adopts [`circle-ir@3.214.0`](https://www.npmjs.com/package/circle-ir) — language-scopes the Rust web-framework extractor so Java `Path`/`Form`/`Query` params are no longer mis-tagged as HTTP sources (cognium-ai#264 "Defect A"). No CLI surface change.
...
- **Fewer XSS false positives** — `request.getAttribute()` / `getSession().getAttribute()`
  reads are no longer flagged as CWE-79 (circle-ir cognium-dev#270). A genuine
  reflected-XSS write still fires.

4509 circle-ir tests pass, 143 CLI tests pass.
...
- **Two Java false-positive classes gone** (circle-ir cognium-ai#247): a
  parameterized `prepareStatement(q.replaceAll(…, "?"))` is no longer
  `sql_injection`, and a JSON API response `.body(…)` is no longer `xss`.
  Genuine concatenated SQL and text/html responses still fire.

4502 circle-ir tests pass, 143 CLI tests pass.
...
- **`sbom` now discovers `pyproject.toml` and `build.gradle`(`.kts`)** in
  addition to package.json / requirements.txt / pom.xml / Cargo.toml / go.mod
  — seven manifest forms across six ecosystems.

4468 circle-ir tests pass, 140 CLI tests pass.
...
- **`SINK_SEVERITY` / `SINK_CWE` now cover all sink types.** Four sink types
  in circle-ir's `SinkType` union — `insecure_storage` (medium, CWE-922),
  `prototype_pollution` (high, CWE-1321), `regex_dos` (high, CWE-1333) and
  `unsafe_memory` (high, CWE-119) — were absent from both maps. A cross-file
  taint flow of one of these types rendered with a defaulted `high` severity
  and an undefined CWE. Both maps are now exhaustive over `SinkType`.

### Notes

- **`ir.types` now includes TypeScript `interface` declarations** (new
  `kind: 'interface'` entries) where earlier versions emitted none. Text/JSON/
  SARIF scan output is unaffected — the CLI does not surface `ir.types`.

4425 circle-ir tests pass, 133 CLI tests pass.
...
Rust cross-file taint (#146) + multi-hop recall, def-precise cross-file
reassignment clobber for Java/JS/Rust/Go (#266), and a #265 JSON.parse
precision lock.

**No CLI-side code changes** — this release only propagates the engine
improvements from the pinned `circle-ir` dependency. Suite 4295 pass,
2 skipped, 1 todo, 0 regressions vs 3.192.0. The cross-file changes touch
shared infrastructure across all six languages; benchmark verification is
the authoritative regression signal — see the circle-ir 3.193.0 changelog.

See the [circle-ir 3.193.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.193.0) for full engine detail.

## [3.192.0] - 2026-07-24

Adopts [`circle-ir@3.192.0`](https://www.npmjs.com/package/circle-ir),
which ships the twelfth #213 slice — bash shell-quote sanitizers
(`printf '%q'` + `${var@Q}` recognized as command_injection /
code_injection sanitizers).
...
See the [circle-ir 3.192.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.192.0) for full engine detail.
...
which ships the tenth #213 slice — modern JS/TS framework sources
(NextJS App Router `req.nextUrl`, Angular ActivatedRoute, Remix
LoaderArgs, SvelteKit event.params/url).

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4269 pass, 2 skipped, 0 regressions vs 3.189.0.

See the [circle-ir 3.190.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.190.0) for full engine detail.

## [3.189.0] - 2026-07-24

Adopts [`circle-ir@3.189.0`](https://www.npmjs.com/package/circle-ir),
which ships the ninth #213 slice — Go web framework sources
(Gin/Echo/Fiber/Chi/Beego/mux) + Go body-binder DFG hooks
(BindJSON/BodyParser/etc.) modeled as opaque-codec destination
re-defs.

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4264 pass, 2 skipped, 0 regressions vs 3.188.0.

See the [circle-ir 3.189.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.189.0) for full engine detail.

## [3.188.0] - 2026-07-24

Adopts [`circle-ir@3.188.0`](https://www.npmjs.com/package/circle-ir),
which ships the eighth #213 slice — NestJS controller param decorator
sources (@Param, @Req, @Session, @Ip, @HostParam, @UploadedFile).

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4258 pass, 2 skipped, 0 regressions vs 3.187.0.

See the [circle-ir 3.188.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.188.0) for full engine detail.

## [3.187.0] - 2026-07-24

Adopts [`circle-ir@3.187.0`](https://www.npmjs.com/package/circle-ir),
which ships the seventh #213 slice — Java sanitizer parity. Rounds
out PreparedStatement setter coverage to the full JDBC 4.x surface,
adds Apache Commons Text / Guava / OWASP HTML sanitizer XSS helpers,
and registers Base64 / Hex / MessageDigest as taint barriers.

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4253 pass, 2 skipped, 0 regressions vs 3.186.0.

See the [circle-ir 3.187.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.187.0) for full engine detail.

## [3.186.0] - 2026-07-24

Adopts [`circle-ir@3.186.0`](https://www.npmjs.com/package/circle-ir),
which ships the sixth #213 slice — JS/Node and Go sanitizer coverage.
Parallel to the 3.185.0 Python slice.

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4246 pass, 2 skipped, 0 regressions vs 3.185.0.

See the [circle-ir 3.186.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.186.0) for full engine detail.
...
which ships the third and fourth slices of #213: JS/TS WebSocket
callback-parameter sources (closes the 3.183.0 deferral) and bash
`read` / `mapfile` / `readarray` / `getopts` builtin sources.

**No CLI-side code changes** — this release only propagates the
engine improvements from the pinned `circle-ir` dependency. Suite
4232 pass, 2 skipped, 0 regressions vs 3.183.0.

See the [circle-ir 3.184.0 changelog](https://www.npmjs.com/package/circle-ir/v/3.184.0) for full engine detail.
...
Engine bump only — adopts
[`circle-ir@3.159.0`](https://www.npmjs.com/package/circle-ir), which
ships **cognium-dev #239 C4 residual** fix:

- **Java `Path.getFileName()` path-traversal sanitizer** — the aisec
  safe-mirror `SafeToctou.java` idiom
  `String leaf = Paths.get(userIn).getFileName().toString(); Paths.get(BASE, leaf);`
  is now correctly recognized. Prior to 3.159.0 this shape fired a
  cross-file `path_traversal` FP in project-mode scans. Fix ships a
  new `findJavaPathGetFileNameSanitizers()` emitter in
  `language-sources-pass` plus a new sanitizer post-filter step (2b)
  in `cross-file-pass` that consults per-file sanitizers before
  emitting cross-file taint paths.

Full details: [`circle-ir@3.159.0`
CHANGELOG](https://github.com/cogniumhq/circle-ir/blob/main/CHANGELOG.md#31590---2026-07-09).
...
Engine bump only — adopts
[`circle-ir@3.148.0`](https://www.npmjs.com/package/circle-ir), which
ships the Java response-writer xss FP fix for the same-file encoder-var
shape (cognium-dev #153).
...
Engine bump only — adopts
[`circle-ir@3.125.0`](https://www.npmjs.com/package/circle-ir) which
closes 3 of the 12 remaining FPs on cognium-dev #216 (Python wrapper
Pattern B: regex-allowlist wrapper, var-aware set-membership xss
guard, and defusedxml import-alias recognition). No CLI surface
changes.

## [3.124.0] - 2026-06-29

Engine bump only — adopts
[`circle-ir@3.124.0`](https://www.npmjs.com/package/circle-ir) which
closes 6 of the 18 remaining FPs on cognium-dev #216 (4 ETE +
2 wrapper) via JS/Java sanitizer-chain recognition. No CLI surface
changes.
