...
- **The C# allowlist character strip is credited (#272, LDAP cluster).** `Regex.Replace(x, "[^a-zA-Z0-9]", "")` deletes every character *not* in the set, so the survivors are exactly the set and the value carries no metacharacter. Credited for `ldap_injection`, `xpath_injection`, `command_injection` and `sql_injection`. Three conditions bound it: the class must be **negated** (a blacklist such as `"[<>]"` proves nothing and is not credited), the surviving set must be metacharacter-free (`"[^a-zA-Z0-9']"` keeps the single quote alive and is **not** credited, because a surviving quote defeats the escaping a query depends on), and the pattern must be a literal with an empty or single safe replacement.
...
- **Finding counts move in both directions**, so re-baseline rather than assuming a one-way shift.
  ...
  - **Fewer findings:** C# loses `path_traversal` on canonicalize-then-contain (#286 C) and `ssrf` on a constant `BaseAddress` with a relative path (#286 D); Java and Python lose `log_injection` where a CRLF strip lives in a helper (#293); C# loses `ldap_injection`, `xpath_injection`, `command_injection` and `sql_injection` where an allowlist character strip is applied (#272).

#### Note on this release's contents vs. its tag

The published `circle-ir@4.9.13` tarball **includes #272**, but the `circle-ir-v4.9.13` tag does not: #272 merged to `main` after the tag was cut, and the npm publish was made from `main`. The tag is left where it is rather than rewritten, and this entry describes what was actually published. Verified by unpacking the published tarball. Nothing else differs between the tag and the artifact.
...
- **More canonical C# sink coverage.** Path traversal: `File.Copy`/`Move`/`Delete`/`Open`/`OpenText`/`Create`, `File.WriteAllLines`/`AppendAllLines`, and `Directory.CreateDirectory`/`Delete`/`GetFiles`/`EnumerateFiles`. Code injection: `Assembly.Load`/`LoadFrom`/`LoadFile` (loading an attacker-named assembly). Class-scoped and taint-gated; a constant path/name never fires. (`Type.GetType`/`Activator.CreateInstance` deliberately not added — reflection over config-driven type names is an FP trap.)
- **C# `ILogger` log injection** (CWE-117, cognium-ai#318). `LogInformation`/`LogWarning`/`LogError`/`LogDebug`/`LogCritical`/`LogTrace` (and the generic `ILogger.Log`) fire `log_injection` when the **message template** is tainted. Only `arg_positions:[0]` (the template) — the structured form `LogInformation("… {Id}", id)` keeps taint in a data argument and stays clean. Mirrors the Java `Logger` sinks; severity low. (This completes the #318 sink families; open_redirect/crlf/nosql shipped 4.7.0, xxe/deser conversion + sink names in 4.7.2.)
...
- **ASP.NET `ControllerBase.File(stream|bytes, contentType)` no longer false-positives as `path_traversal`** (cognium-ai#326 Child B). The Java `new File(path)` sink was unscoped and over-matched the .NET `File(...)` result helper (which returns a file, not a path). Excluded for C# — .NET has no `new File()` (path sinks are `File.ReadAllText`/`OpenRead`/`FileStream`, covered separately, and still fire).
- **A tainted variable name appearing only inside a string literal no longer taints the assignment** (cognium-ai#326 Child C). The C#/Java text-scan (`buildJavaTaintedVars`) matched a tainted var name anywhere in the RHS, so `var cmd = new SqlCommand("… WHERE n=@n")` — a **parameterized** query whose literal merely contains the parameter name `n` — spuriously tainted `cmd` and fired `sql_injection` at a later `cmd.Execute*()`. The RHS is now literal-stripped before matching (keeping `$"…{expr}"` interpolation contents), mirroring the #271 object-carry. Genuine concatenation/interpolation into the command still fires. 0 flow delta on OWASP Java 2740 + SecuriBench 125 + BenchmarkPython 1230.
- **`hardcoded-credential` no longer fires when a string value equals its own declaring identifier or is an XML namespace** (cognium-ai#326 Child D). `LicenseReportPassword = "LicenseReportPassword"` (a config key name), `ViewPasswordBtn`, and OOXML namespace URIs containing `password` are not secrets. Genuine high-entropy secrets still fire.
...
- **Rust `std::process::Command::…output()` no longer emits a spurious `xss` sink** (cognium-dev#146). The classless `output` XSS sink (a Java template-engine pattern) matched Rust's process-`output()`, which already fires `command_injection` — so a cross-file `Command` flow was double-labeled (and could surface as XSS). Rust is now excluded from that XSS sink. (Cross-file taint for Rust & TypeScript — the subject of #146 — is otherwise already working and locked by `xfile-rust-ts-146.test.ts`: controller `http_param` → imported-helper `command_injection`/`path_traversal`/`sql_injection`/`ssrf`.)
- **Parameterized ORM / query-builder finders no longer produce a CWE-668 `external_taint_escape` false positive** (cognium-ai#279 R-1). A tainted value inside a `where`/filter object passed to `findMany`/`findFirst`/`findUnique`/`findOne`/`findAll`/`findByPk`/`where`/`andWhere`/`orWhere`/`whereIn`/`first`/`bind` (Prisma, Sequelize, TypeORM, Strapi, knex, drizzle) is *parameterized* by the driver, not injected — so these no longer fall through to the low-confidence escape fallback (same treatment as Go's parameterized `Query`/`Exec`). The raw/unsafe siblings (`queryRaw`, `$queryRawUnsafe`, `whereRaw`, `raw`, `find`) are intentionally excluded and still fire, and configured `sql_injection`/`command_injection`/`nosql_injection` sinks are unaffected. This was the largest residual JS/TS FP class (~53% of cognium-ai#279). 0 flow delta on OWASP Java 2740 + SecuriBench 125 + BenchmarkPython 1230.
...
- **C# XXE hardening is now credited (cognium-dev#272, partial).** `XmlResolver = null` and `DtdProcessing = Prohibit|Ignore` are the documented XXE mitigations — once either is set, an external entity cannot resolve — so a hardened parse (`XmlDocument.Load`/`LoadXml`, `XmlReader.Create`) no longer fires `xxe`. Deterministic config markers (not guard/dominator inference), credited file-wide, mirroring the file-level deserialization safety gate; an unhardened parse still fires. (The remaining #272 clusters — `ProcessStartInfo` non-shell argv, SSRF host-allowlist, LDAP char-strip, XPath validation guards — need dominator/guard analysis and are gated on the Juliet-C# corpus, cognium-ai#285.)
...
- **Three new C# taint sink categories** (cognium-dev#273/#275) — all taint-gated, so a constant argument never fires: **open_redirect** (CWE-601) — ASP.NET `return Redirect(url)` / `Results.Redirect(url)` / `RedirectPermanent` (classless; `LocalRedirect` deliberately excluded — it rejects non-local URLs and is the mitigation); **crlf / HTTP response-header injection** (CWE-113) — `Response.AddHeader`/`AppendHeader` with a tainted value; **nosql_injection** (CWE-943) — MongoDB `BsonDocument.Parse(userJson)`. (The remaining #273/#275 items are held for the Juliet-C# corpus: `log_injection`/`format_string`/`ssti` are generic-method-name FP traps, the tier-2 config passes — `insecure-cookie`/`tls-verify-disabled`/`jwt-verify-disabled` — need C# branches, and Dapper/Flurl/RestSharp/Blazor are ecosystem sinks best bounded by the corpus, cognium-ai#285.)
- **`missing-public-doc` (CWE-1059, maintainability) now covers C#** (cognium-dev#274, Q-25). An undocumented `public` type or method is flagged; `///` XML-doc comments, and `private`/`internal`/`protected` members, stay clean. (The other #274 items are out of scope here: `god-class`/`feature-envy` are deliberately off the default pipeline, `magic-numbers` has no pass yet, and the McCabe/maintainability-index metrics depend on the C# CFG, which is still a stub — tracked for the Juliet-C# corpus milestone, cognium-ai#285.)
