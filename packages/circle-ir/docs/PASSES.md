...
| 95 | `xml-entity-expansion` | CWE-611 / CWE-776 | error | shipped | XML parser created without external-entity / entity-expansion safeguards (XXE / billion-laughs). **CWE is per API (#426):** Java JAXP factories and `lxml.etree` → CWE-611 (external entities resolve by default); `xml.etree.ElementTree` → CWE-776 (never resolves external entities, so only expansion remains); JS `libxmljs` `noent: true` → CWE-611. Detection: Java `SAXParserFactory/DocumentBuilderFactory/XMLInputFactory/SchemaFactory/TransformerFactory.newInstance()` without `disallow-doctype-decl`/`external-general-entities`/`SUPPORT_DTD`/`ACCESS_EXTERNAL_DTD`/`setXIncludeAware(false)`/`setExpandEntityReferences(false)` in file; hardening evidence silences the file EXCEPT for a factory that provably never receives it — chained (`Factory.newInstance().newDocumentBuilder()`) or a local that builds a parser, with no hand-off, no return, and no hardening-capable setter on any receiver in its method (`TransformerFactory` additionally needs a `StreamSource` in the method) — so one hardened factory no longer masks an unhardened sibling; Python `lxml.etree.parse/fromstring/XML` and `xml.etree.ElementTree.parse/fromstring` without `defusedxml` or `resolve_entities=False`. High severity. Issue #86, Sprint 6. (sprint 78: JS `libxml(js).parseXml(buf, { noent: true })` / `parseXmlString(..., { noent: true })` enables external-entity resolution — XXE / billion-laughs) |
...
| 117 | `insecure-deserialization-config` | CWE-502 | error | shipped | Java-only pattern pass (call-shape match, not a taint flow — like `weak-crypto`). Flags XStream configured with a grant-all type permission: `addPermission(AnyTypePermission.ANY)` or `addPermission(new AnyTypePermission())` (incl. fully-qualified `com.thoughtworks.xstream.security.AnyTypePermission`). `AnyTypePermission` re-enables XStream's pre-1.4.10 grant-everything default, so a later `fromXML(untrusted)` can instantiate arbitrary gadget types (CVE-2013-7285, CVE-2020-26217, CVE-2021-21345, …). Emits a `SastFinding` at the `addPermission` line (severity `high`, level `error`) — deterministic and independent of any source→sink flow, complementing the existing `fromXML`/`unmarshal` CWE-502 taint sink (which fires at the deserialization call site only with a reachable source). Precise: the argument is unambiguously XStream's security class, so the secure shapes (`NoTypePermission.NONE`, `WildcardTypePermission(...)`, `allowTypes(...)`) do NOT match — 0 findings across the 2865-file OWASP Java + SecuriBench corpus. Fix hint points to an explicit allow-list. Guarded on `disabledPasses: ['insecure-deserialization-config']`. Issue #225 — 4.6.0 |
| 118 | `param-source-flow-gate` | — (flow-gating, no CWE) | — | shipped | Flow-gating pass (emits no findings). Drops `taint.flows` whose only source is a bare function parameter (`interprocedural_param`) unless `analyze(..., { speculativeParamSources: true })` is set — honouring the #267 contract that bare-parameter sources are speculative and off by default, for every sink type. Exempt: Java / C# (entry-point classifier #128; C# model-binding params #273), route decorators / annotations, Rust route attributes, Rust handlers returning a web response type, and flows whose source line also carries a real source. Sources are untouched, so cross-file resolution and co-occurrence scoring are unchanged. Measured: 0 TP lost on OWASP / SecuriBench / JS / Go benchmarks, 1/149 BenchmarkPython (mis-attributed), 1 Rust inline; ~99% of bare-parameter flows on a 41-repo real-world sample removed (19/20 sampled were noise). Pipeline slot: right after `InterproceduralPass`. Guarded on `disabledPasses: ['param-source-flow-gate']`. Issue #292 |
