...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.187.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Seven slices shipped, sanitizer_kind grid now complete across Python/JS/Go/Java.** Slices 1-4 (3.181.0–3.184.0): source coverage — Lambda events, WebSocket handlers (Python/Go/Java + JS/TS), bash builtins. Slices 5-7 (3.185.0–3.187.0): sanitizer_kind grid — Python (urllib.parse, bleach, django.utils.html, jinja2/flask/saxutils, re.escape, SQLAlchemy/psycopg2) + narrow bare-import symmetry fix in matchesSanitizerPattern; JS/Node (he, sanitize-html, xss, xss-filters, entities, sqlstring, pg-format, crypto.randomUUID, URLSearchParams); Go (regexp.QuoteMeta, bluemonday, net.ParseIP, sql.Named); Java (full JDBC 4.x PreparedStatement setters — 20+ types previously missing including setObject/setDate/setBigDecimal/setBytes/setBlob/setTimestamp/setURL/setSQLXML, plus TypedQuery.setParameter and MapSqlParameterSource.addValue; Apache Commons Text escapeJson/EcmaScript/Xml11/Csv/Xsi/Java; Guava HtmlEscapers; OWASP PolicyFactory.sanitize; Base64.Encoder.encodeToString/encode + bare aliases; Hex.encodeHexString + bare; MessageDigest.digest). All sanitizer entries include external_taint_escape in removes. 41 tests total across slices. Also discovered (3.183.0 investigation): the "nested-property access `event.pathParameters.id` falls through" claim in the first slice's test comment was inaccurate — the actual gap was reach-map exclusion of http_path→command_injection / http_header→command_injection (deliberate FP-control policy), not the JS matcher. | Remaining: go/ts source parity cells (74+74), remaining channel cells beyond WebSocket/Lambda (GraphQL/gRPC already covered pre-slice-1), bash sanitizer coverage, external harness verification. |
