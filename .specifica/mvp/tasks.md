...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.189.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Nine slices shipped.** Slices 1-4: source coverage — Lambda events, WebSocket, bash builtins. Slices 5-7: sanitizer_kind grid complete across Python/JS/Go/Java. Slice 8: NestJS decorators. Slice 9 (3.189.0, 433715c): Go web framework sources — Gin (c.Query/Param/PostForm/GetHeader/Cookie/FormFile/MultipartForm + body binders BindJSON/ShouldBindJSON/Bind/BindQuery/BindUri), Echo (c.QueryParam/QueryParams/FormValue/FormParams/FormFile), Fiber (c.Query/Params/FormValue/FormFile/Cookies/Get/Body/BodyParser/QueryParser/ParamsParser — class Ctx), Chi (chi.URLParam/URLParamFromCtx via bare-import resolution), Beego (ctx.Input.Query/Param/Header/Cookie), Gorilla (mux.Vars). Body-binder DFG hooks: extended GO_OPAQUE_CODEC_METHODS (originally for json.Unmarshal per #243 shape 3) with all Bind*/ShouldBind*/BodyParser/QueryParser/ParamsParser variants so `c.BodyParser(&q)` links body → q via DFG re-def. Class-matching relies on the Go local-receiver resolver shipped in #240 3.177.0. 52 tests total across all slices. | Remaining: TS source parity cells, bash sanitizer coverage, external harness verification. |
