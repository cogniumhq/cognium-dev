...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.185.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Five slices shipped.** 3.181.0 (ecfdabb): Lambda `event.*` sources. 3.183.0 (7311efe): WebSocket handler payload sources for Python/Go/Java. 3.184.0 (3b920e4): JS/TS `ws.on('message', cb)` callback-parameter sources + bash `read`/`mapfile`/`readarray`/`getopts` builtins. 3.185.0 (3484c56): Python sanitizer coverage — urllib.parse quote/quote_plus/urlencode, bleach.linkify, django.utils.html.escape/strip_tags, jinja2/flask escape, xml.sax.saxutils, re.escape (redos+code_injection), SQLAlchemy/psycopg2 SQL helpers. Also fixed narrow symmetry gap in `matchesSanitizerPattern`: bare-imported class-scoped sanitizers now credit via `call.resolution.target` tail-match (mirrors source-side). Deliberately narrower than source-side to keep Java `Path.normalize` semantic scope intact per tests/issue-216-pattern-b-java.test.ts. 26 tests total across slices. Also discovered (3.183.0 investigation): the "nested-property access `event.pathParameters.id` falls through" claim in the first slice's test comment was inaccurate — the actual gap was reach-map exclusion of http_path→command_injection / http_header→command_injection (deliberate FP-control policy), not the JS matcher. | Remaining: sanitizer_kind grid (further cells beyond the ~15 Python entries just shipped), go/ts parity cells, external harness verification. |
