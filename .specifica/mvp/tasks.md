...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.186.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Six slices shipped.** 3.181.0 (ecfdabb): Lambda `event.*` sources. 3.183.0 (7311efe): WebSocket handler payload sources for Python/Go/Java. 3.184.0 (3b920e4): JS/TS `ws.on('message', cb)` callback-parameter sources + bash `read`/`mapfile`/`readarray`/`getopts` builtins. 3.185.0 (3484c56): Python sanitizer coverage + narrow bare-import symmetry fix in matchesSanitizerPattern. 3.186.0 (51e9f91): JS/Node sanitizer coverage (he, sanitize-html, xss, xss-filters, entities, sqlstring, pg-format, crypto.randomUUID, URLSearchParams) + Go sanitizer coverage (regexp.QuoteMeta, bluemonday.Sanitize class+bare, net.ParseIP, sql.Named). All new sanitizer entries include external_taint_escape in removes so sanitizer-at-sink-line doesn't spuriously flag CWE-668. 34 tests total across slices. Also discovered (3.183.0 investigation): the "nested-property access `event.pathParameters.id` falls through" claim in the first slice's test comment was inaccurate — the actual gap was reach-map exclusion of http_path→command_injection / http_header→command_injection (deliberate FP-control policy), not the JS matcher. | Remaining: Java sanitizer parity (already fairly complete), remaining go/ts source parity cells, external harness verification. |
