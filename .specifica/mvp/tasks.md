...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.190.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Ten slices shipped.** Slices 1-4: source coverage (Lambda, WebSocket, bash builtins). Slices 5-7: sanitizer_kind grid complete across Python/JS/Go/Java. Slice 8: NestJS decorators. Slice 9: Go web framework sources (Gin/Echo/Fiber/Chi/Beego/Gorilla + body-binder DFG hooks). Slice 10 (3.190.0, cc4ea3b): modern JS/TS framework sources — NextJS App Router (req.nextUrl property_tainted), Angular (route.snapshot/paramMap/queryParamMap property_tainted), Remix/SvelteKit/Astro/Bun/Deno (regex entries in JS_TAINTED_PATTERNS for event.params/url/request, route.snapshot.*, generic .searchParams.get catch-all). Generic `params`/`url` unqualified property names deliberately excluded to avoid over-firing. 57 tests total across all slices. | Remaining: bash sanitizer coverage, external harness verification. |
