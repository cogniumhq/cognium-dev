...
### GitHub issue ledger (as of 2026-07-23, post-3.180.0 release)
...
| #261 | capability | Extend `dependencyContext` to Gradle / npm / Python / Cargo (extends #258) | **Released 3.179.0:** Gradle direct-form (a22c261) + Python PyYAML (ade4a3f). **Released 3.180.0:** Rust `Cargo.toml` plumbing (354b25c, no gate consumer — same finding as npm, no clean version-safety story for `serde_yaml`/`bincode`), Gradle version-catalog `libs.versions.toml` (e9640b1), Gradle `constraints{}` + `version{strictly/require/prefer}` sub-block form (0e21732). | Deferred: npm `package.json` (may stay deferred pending real FP-driving case), `platform(...)` BOM (deliberate no-op — BOM version ≠ artifact version), `subprojects {}` (already handled by direct-form regex, verified by test) |
...
| #264 | FN | `format_string` (CWE-134) coverage audit + additions (spun out of #240) | **Released 3.179.0:** 8 Java+Go sink additions (a76af04). **Released 3.180.0:** Logger classification decided (stays `log_injection`/CWE-117, in-code note at `config-loader.ts:1731`, no new sinks — f351687); new `PythonReceiverTaintFormatPass` emits `format_string` findings for `taintedFmt.format(...)` shapes (eae3391, 7 tests). | Still deferred: Python `%`-operator LHS-taint (needs new AST scan over `binary_operator` nodes — genuinely bigger scope); variant-coverage matrix rerun (external corpora, same as #262) |
