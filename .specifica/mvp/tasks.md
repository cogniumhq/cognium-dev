...
### GitHub issue ledger (as of 2026-07-23, post-#243 close, post-3.184.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Four slices shipped.** 3.181.0 (ecfdabb): AWS Lambda / API Gateway `event.*` sources (10 property patterns + 6 JS_TAINTED_PATTERNS regex entries; JS/TS + Python). 3.183.0 (7311efe): WebSocket handler payload sources for Python (FastAPI/Starlette/Django Channels), Go (gorilla/nhooyr), Java (Jakarta WebSocket + Spring STOMP) — 10 DEFAULT_SOURCES + 3 PYTHON_TAINTED_PATTERNS regexes. 3.184.0 (3b920e4) parts A+B: (A) JS/TS `ws.on('message', cb)` callback-parameter sources via `findJavaScriptCallbackParamSources` + `buildJavaScriptTaintedVars` seed extension, event allowlist message/text/binary, `err`/`error` params excluded; closes the 3.183.0 deferral. (B) Bash stdin/CLI builtin sources — `read` / `mapfile` / `readarray` / `getopts` with per-flag arg-consumption enumeration; function-definition FP-guard. 20 tests total for these slices. Also discovered (3.183.0 investigation): the "nested-property access `event.pathParameters.id` falls through" claim in the first slice's test comment was inaccurate — the actual gap was reach-map exclusion of http_path→command_injection / http_header→command_injection (deliberate FP-control policy), not the JS matcher. | Remaining: sanitizer_kind grid (84 cells: 7 sanitizer categories × 4 language categories), remaining go/ts parity cells, external harness verification. |
