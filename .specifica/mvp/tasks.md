...
### GitHub issue ledger (as of 2026-07-24, post-#243 close, post-3.191.0)
...
| #213 | coverage | Taint coverage extension — 512 cells (go/ts/bash + channels + kinds) | **Eleven slices shipped.** Slices 1-4: source coverage (Lambda, WebSocket, bash builtins). Slices 5-7: sanitizer_kind grid complete across Python/JS/Go/Java. Slice 8: NestJS decorators. Slice 9: Go web framework sources. Slice 10: modern JS/TS frameworks (NextJS/Angular/Remix/SvelteKit). Slice 11 (3.191.0, d4d39ea): async Python — extended aiohttp (request.rel_url/remote/raw_headers/transport) + Quart (Request.get_json/get_data/form/files) + PYTHON_TAINTED_PATTERNS regex extension. 62 tests total across all slices. | Remaining: bash sanitizer coverage, external harness verification. |
